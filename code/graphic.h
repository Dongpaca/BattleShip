...
Dx = (X2-X1); // obliczenie sk©øadowej poziomej
Dy = (Y2-Y1); // obliczenie sk©øadowej pionowej

TwoDx = Dx + Dx; // podwojona sk©øadowa pozioma
TwoDy = Dy + Dy; // podwojona sk©øadowa pionowa
...
if(Dx < 0) // jesli sk©øadowa pozioma jest ujemna
  {
  Xinc = -1; // to b?ziemy si?"cofa? (krok ujemny)
  Dx = -Dx;  // zmieniamy znak sk©øadowej na dodatni
  TwoDx = -TwoDx; // jak r?nie?podwojonej sk©øadowej
  }

if (Dy < 0) // je©«li sk©øadowa pionowa jest ujemna
  {
  Yinc = -1; // to b?ziemy si?"cofa? (krok ujemny)
  Dy = -Dy; // zmieniamy znak sk©øadowej na dodatki
  TwoDy = -TwoDy; // jak r?niez podwojonej sk©øadowej
  }

GLCD_SetPixel(X1,Y1, color); // stawiamy pierwszy krok (zapalamy pierwszy piksel)

if ((Dx != 0) || (Dy != 0)) // sprawdzamy czy linia sk©øada si?z wi?ej ni?jednego punktu ;)
  {
  // sprawdzamy czy sk©øadowa pionowa jest mniejsza lub r?na sk©øadowej poziomej
  if (Dy <= Dx) // je©«li tak, to idziemy "po iksach"
    ...
    do // ruszamy w drog?
      ...
      TwoDxAccumulatedError += TwoDy; // a tu dodajemy podwojon?sk©øadow?pionow?
      if(TwoDxAccumulatedError > Dx)  // je©«li TwoDxAccumulatedError jest wi?szy od Dx
        {
       ...
       }while (CurrentX != X2); // idziemy tak d©øugo, a?osi©ögniemy punkt docelowy
     }
